...
INSERT INTO DRIVERS_TBL VALUES('DR003', '홍길동', '010-1234-1254', 'M');
INSERT INTO DRIVERS_TBL VALUES('DR004', '전우치', '010-4567-1254', 'M');
...
--2. 대리기사가 선택한 대리운전 결정 리스트를 보여주세요(5점)
        ...
        SELECT T1.DR_ID, T1.DR_NAME, T1.DR_TEL, T3.R_TEL, T3.R_STR, T3.R_DEST,T3.R_PAY
        FROM DRIVERS_TBL T1, DRIVER_RST_TBL T2, , RESERVATION_TBL T3
        WHERE T1.DR_ID = T2.DR_ID AND
        T3.R_ID = T2.R_ID;
        
--3. 이용자중 가장많이 대리운전을 예약한 사용자의 연락처를 찾아주세요 (5점) -- BASIC JOIN + GROUP BY
        
        SELECT * FROM
        (
            SELECT COUNT(*), R_TEL, RANK() OVER(ORDER BY COUNT(*) DESC) AS RNK
            FROM RESERVATION_TBL
            GROUP BY R_TEL
        )
        WHERE RNK = 1;
...
--4. 대리운전 기사들의 수익을 가장많은 기사 부터 차례대로 순위를 붙여서 보여 주세요(5점)
        XXXXX
        SELECT T2.DR_ID, SUM(T1.R_PAY)
        FROM RESERVATION_TBL T1, FINISH_DRIVE_TBL T2
        WHERE T1.R_ID = T2.R_ID
        GROUP BY T2.DR_ID
        ORDER BY T1.R_ID;

        SELECT DR_ID, DR_NAME, SUM(PROFIT),
            RANK() OVER(ORDER BY SUM(PROFIT) DESC) AS RNK
        FROM
        (
        SELECT T1.DR_ID, T1.DR_NAME, T3.R_PAY,
            CASE
                WHEN T3.R_PAY >= 20000 THEN T3.R_PAY * 0.8
                ELSE T3.R_PAY * 0.9
            END AS PROFIT
...
--7. 2018년 5월 2일부터 5월 5일까지 대리기사가 결정된 건수, 정상적으로 완료된 건수, 취쇠된 건수를 보여주세요(5점)
        ...
        SELECT A.B_DATE, NVL(B.R_CNT, 0), NVL(B.F_CNT, 0), NVL(B.C_CNT, 0)
        FROM
        (
            SELECT TO_CHAR(TO_DATE('2018-05-01') + (LEVEL - 1), 'YYYY-MM-DD') AS B_DATE
            FROM DUAL
            CONNECT BY LEVEL <= 30
        ) A,
        (
            SELECT TO_CHAR(T3.F_DATE, 'YYYY-MM-DD') AS S_DATE
                , COUNT(*) AS R_CNT
                , SUM(DECODE(F_GUBUN, 1, 1, 0)) AS F_CNT
                , SUM(DECODE(F_GUBUN, 0, 1, 0)) AS C_CNT
            FROM DRIVERS_TBL T1, DRIVER_RST_TBL T2, FINISH_DRIVE_TBL T3
            WHERE T1.DR_ID = T2.DR_ID
            AND T2.DR_ID = T3.DR_ID AND T2.R_ID = T3.R_ID
            GROUP BY TO_CHAR(T3.F_DATE, 'YYYY-MM-DD')
        ) B
        WHERE A.B_DATE = B.S_DATE(+)
        ORDER BY A.B_DATE ASC;
...
--10. 대리운전비가 가장 비싼 대리운전을 보여주세요(5점)
       ...
       XXXXXX
       SELECT T3.DR_NAME, T1.R_STR, T1.R_DEST, T3.DR_TEL, T1.R_PAY
       FROM RESERVATION_TBL T1, DRIVERS_TBL T3, FINISH_DRIVE_TBL T2
       WHERE  T2.DR_ID(+) = T3.DR_ID AND
       T1.R_ID = T2.R_ID AND
       T1.R_PAY = (SELECT MAX(R_PAY)
                    FROM RESERVATION_TBL);
